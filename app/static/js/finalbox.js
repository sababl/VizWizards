...
d3.csv("/static/data/box_life_expectancy_by_region.csv").then(data => {
    ...
    const boxWrapper = d3.select("#box-wrapper");
    const tooltip = d3.select("#box-tooltip");
    ...
    data.forEach(d => {
        ...
        g.append("rect")
            ...
            .on("mousemove", (event) => {
                // Use d3.pointer to get mouse [x, y] in container coordinates
                const [mouseX, mouseY] = d3.pointer(event, boxWrapper.node());
