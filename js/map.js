...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const svg = d3.select("#total-emissions")
        .append("svg")
        .attr("width", width)
        .attr("height", height)
    ...
    const colorScale = d3.scaleSequential(d3.interpolateReds)
        .domain([0, 70]); // Adjust the domain to fit your data range
    ...
    d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(function(geoData) {
        ...
        d3.csv("../data/sorted_emissions_one_year.csv").then(function(data) {
            ...
            data.forEach(d => {
                if (d.Year == 1996) { // Only using data from the year 1996
            ...
            svg.selectAll("path")
                ...
                .on("mouseover", function(event, d) {
                    ...
                    if (emissionData) {
                        ...
                        tooltip.html(`<strong>${emissionData.country}</strong><br>Emissions: ${emissionData.emission} tons per capita`)
    ...
    d3.select("body").append("button")
        ...
        .on("click", function() {
            currentProjection = (currentProjection === mercatorProjection) ? orthographicProjection : mercatorProjection;
